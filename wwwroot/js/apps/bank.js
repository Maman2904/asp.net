"use strict";
// Class definition
var DeleteBank = async function(id) {
	Swal.fire({
		...
		if (result.isConfirmed) {
			let doDelete = await fetch("/"+lang+"/TableManagement/DeleteBank/"+id, {
				method: 'DELETE'
			});
...
var KTDatatableAjaxTable = (function () {
  ...
  var datatableInitialize = function () {
    var datatableColumn = [
      ...
      },
      {
        ...
        template: function (row) {
          ...
          // }
            return '\
            <a href=/'+lang+'/TableManagement/Bank/Edit/'+row.bank_id+' class="btn btn-sm btn-clean btn-icon mr-2" title="Lihat Detil">\
            ...
            <button'+ ' class="btn btn-sm btn-clean btn-icon mr-2 deleteBank" title="Delete" onclick=DeleteBank("' +row.bank_id+'") data-bank_id='+row.bank_id+'>\
							<span class="svg-icon svg-icon-md">\
    ...
    datatable = $("#kt_datatable_bank_all").KTDatatable({
      data: {
        type: "remote",
        source: {
          read: {
            method: "GET",
            url: "/" + lang + "/TableManagement/all_bank_datatable",
            map: function (raw) {
              var dataSet = raw;
