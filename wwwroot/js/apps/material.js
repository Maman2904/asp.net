"use strict";
// Class definition
var DeleteMaterial = async function(id) {
	console.log("masukkk")
	Swal.fire({
		...
		if (result.isConfirmed) {
			let doDelete = await fetch("/"+lang+"/TableManagement/DeleteMaterial/"+id, {
				method: 'DELETE'
			});
      ...
      location.reload();
				...
				$('#kt_datatable_material_all').KTDatatable().reload();
			} else {
				Swal.fire({
...
var KTDatatableAjaxTable = (function () {
  ...
  var datatableInitialize = function () {
    var datatableColumn = [
      ...
      },
      {
        ...
        template: function (row) {
          ...
          // }
            return '\
            <a href=/'+lang+'/TableManagement/Material/Edit/'+row.material_id+' class="btn btn-sm btn-clean btn-icon mr-2" title="Lihat Detil">\
            ...
            <button'+ ' class="btn btn-sm btn-clean btn-icon mr-2 deleteMaterial" title="Delete" onclick=DeleteMaterial("' + row.material_id + '") data-material_id='+row.material_id+'>\
							<span class="svg-icon svg-icon-md">\
    ...
    datatable = $("#kt_datatable_material_all").KTDatatable({
      data: {
        type: "remote",
        source: {
          read: {
            method: "GET",
            url: "/" + lang + "/TableManagement/all_material_items_datatable",
            map: function (raw) {
              var dataSet = raw;
