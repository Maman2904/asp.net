"use strict";
// Class definition
var DeleteCostCenter = async function(id) {
	console.log("masukkk")
	Swal.fire({
		...
		if (result.isConfirmed) {
			let doDelete = await fetch("/"+lang+"/TableManagement/DeleteCostCenter/"+id, {
				method: 'DELETE'
			});
      ...
      location.reload();
				...
				$('#kt_datatable_cost_center_all').KTDatatable().reload();
			} else {
				Swal.fire({
...
var KTDatatableAjaxTable = (function () {
  ...
  var datatableInitialize = function () {
    var datatableColumn = [
      ...
      },
      {
        ...
        template: function (row) {
          ...
          // }
            return '\
            <a href=/'+lang+'/TableManagement/CostCenter/Edit/'+row.cc_id+' class="btn btn-sm btn-clean btn-icon mr-2" title="Lihat Detil">\
            ...
            <button'+ ' class="btn btn-sm btn-clean btn-icon mr-2 deleteCostCenter" title="Delete" onclick=DeleteCostCenter(' + row.cc_id + ') data-cc_id='+row.cc_id+'>\
							<span class="svg-icon svg-icon-md">\
    ...
    datatable = $("#kt_datatable_cost_center_all").KTDatatable({
      data: {
        type: "remote",
        source: {
          read: {
            method: "GET",
            url: "/" + lang + "/TableManagement/all_cc_items_datatable",
            map: function (raw) {
              var dataSet = raw;
